...
import '@styles/node/autonomys/AutoMenu.css';
...
const AutoMenu = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(currentPath === '' || currentPath === 'autonomys' ? 'overview' : currentPath);
  }, [location]);

  const handleTabClick = (tab) => {
    const targetPath = tab.path ? `/${tab.path}` : '';
    setActiveTab(tab.name.toLowerCase());
    navigate(`/services/testnet/autonomys${targetPath}`, { replace: true });
  };
