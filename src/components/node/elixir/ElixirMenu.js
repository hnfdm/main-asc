...
import '@styles/node/elixir/ElixirMenu.css';
...
const ExilirMenu = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(currentPath === '' || currentPath === 'elixir' ? 'overview' : currentPath);
  }, [location]);

  const handleTabClick = (tab) => {
    const targetPath = tab.path ? `/${tab.path}` : '';
    setActiveTab(tab.name.toLowerCase());
    navigate(`/services/testnet/elixir${targetPath}`, { replace: true });
  };
