...
import RpcStatusBox from './RpcStatusBox'; // Pastikan jalur impor sesuai
import '@styles/node/nillon/NillonMenu.css';
...
const NillonMenu = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(currentPath === '' || currentPath === 'nillon' ? 'overview' : currentPath);
  }, [location]);

  const handleTabClick = (tab) => {
    const targetPath = tab.path ? `/${tab.path}` : '';
    setActiveTab(tab.name.toLowerCase());
    navigate(`/services/testnet/nillon${targetPath}`, { replace: true });
  };
