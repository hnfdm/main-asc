...
import '@styles/node/t3rn/TernMenu.css';
...
const TernMenu = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(currentPath === '' || currentPath === 't3rn' ? 'overview' : currentPath);
  }, [location]);

  const handleTabClick = (tab) => {
    const targetPath = tab.path ? `/${tab.path}` : '';
    setActiveTab(tab.name.toLowerCase());
    navigate(`/services/testnet/t3rn${targetPath}`, { replace: true });
  };
